...
def load_model(pred_config, args, device):
    ...
    try:
        model = MODEL_CLASSES[args.model_type][1].from_pretrained(
            args.model_dir, args=args, intent_label_lst=get_intent_labels(args)
        )
        model.to(device)
        model.eval()
...
def predict(pred_config):
    ...
    dataset = convert_input_file_to_tensor_dataset(lines[0], pred_config, args, tokenizer, pad_token_label_id)
